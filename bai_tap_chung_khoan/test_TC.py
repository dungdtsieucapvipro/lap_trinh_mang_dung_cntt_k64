...
with sync_playwright() as p:
    ...
    # Tìm tất cả các phần tử có class cho giá trị trần
    cells = page.query_selector_all(".ag-cell.ag-cell-not-inline-editing.ag-cell-normal-height.ag-cell-color-ref.ag-cell-bg-highlight.cursor-pointer.ag-cell-value")

    if cells:
        for index, cell in enumerate(cells):
            content = cell.inner_text()  # Lấy nội dung của từng ô
            print(f"Nội dung ô TC thứ {index + 1}:", content)
    else:
        print("Không tìm thấy ô nào có class 'ag-cell.ag-cell-not-inline-editing.ag-cell-normal-height.ag-cell-color-ref.ag-cell-bg-highlight.cursor-pointer.ag-cell-value'.")
